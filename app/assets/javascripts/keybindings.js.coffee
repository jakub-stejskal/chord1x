...
handleKeyBindings = ->
  ...
  $('input[data-keybinding]').not('[type="submit"]').each (i, el) ->
  ...
  # Hotkey binding to inputs with 'data-keybinding' attribute
  # Click submit input when hotkey pressed
  $('input[data-keybinding][type="submit"]').each (i, el) ->
    Mousetrap.bind $(el).data('keybinding'), (e) ->
      el.click()
      if e.preventDefault
        e.preventDefault()
  ...
  # ctrl+enter is triggered even in inputs
  _oldStopCallback = Mousetrap.stopCallback
  Mousetrap.stopCallback = (e, element, combo) ->
    if combo == 'ctrl+enter'
      false
    else
      _oldStopCallback e, element, combo

  # Song selection with j, k, enter
  Mousetrap.bind 'j', (e) ->
    current = $('tr.selected')
    current.removeClass('selected')
    current.nextOrFirst().addClass('selected')
  Mousetrap.bind 'k', (e) ->
    current = $('tr.selected')
    current.removeClass('selected')
    current.prevOrLast().addClass('selected')
  Mousetrap.bind 'enter', (e) ->
    $('tr.selected a')[0]?.click()
